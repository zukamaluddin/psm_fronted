...
export default class ListGenerik extends React.Component {
    selectedDataAssign = [];
    constructor(props) {
        super(props);
        this.state = {
    ...
    loadData = async () => {
        let result = await API.listGenerik(this.state.set);
        this.setState({
            data: result.data,
    ...
    fetchData(state, instance) {
        ...
        for (var x in state.filtered) {
            ...
            if (state.filtered[x].id === 'kod') {
                filterKod = state.filtered[x].value
            }
        }
                ...
                <CardBody>
                    <div>
                        <Button style={{width: '140px'}}
                                onClick={() => {
                                    this.props.history.push('/setting/registerGenerik');
                                    setTimeout(function () {
                                        settingMenu.changeActiveLinkTo('#/setting/registerGenerik');
                                    }.bind(this),);
                                }}
