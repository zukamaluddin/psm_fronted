...
export default class ResultList extends React.Component {
    ...
    onMonth = (year, month) => {
        let tarikh = new Date(year, month, 0);
        ...
        fetch(global.ipServer + `report/find_monthly_report?` +
            ...
            this.setState({showResult: 'monthly', dataMonthly: result});
            reportMenu.changeActiveLinkLabel('Bulanan');
        });
    };
    onAlatan = (year, jenis) => {
        let tarikh = new Date(year, 0, 1);
    ...
    render() {
        const {laporan, jumlahKutipan, payment, resit, kutipan} = this.state;
        ...
        let idx = 0;
        const months = [{1: 'Januari'}, {2: 'Februari'}, {3: 'Mac'}, {4: 'April'}, {5: 'Mei'}, {6: 'Jun'}, {7: 'Julai'}, {8: 'Ogos'}, {9: 'September'}, {10: 'October'}, {11: 'November'}, {12: 'Disember'}];

        if (this.state.showResult === 'monthly') {
            return (
                <Redirect to={{
                    pathname: '/report/monthly',
                    state: {
                        searchResult: this.state.dataMonthly,
                        searchQuery: {
                        ...
                        <CardBody>
                            ...
                            >
                                <Row>
                                    ...
                                    <Col md={12}><b className="mr-2">Pecahan mengikut bulan:</b>
                                        {
                                            months.map((value, index) => {
                                                return (
                                                    <div className="mb-2 mr-2 badge badge-pill badge-secondary"
                                                         ...
                                                         onClick={(dataEl) => {
                                                             this.onMonth(year, parseInt(Object.keys(value)[0]))
                                                         }}
                                                    >{Object.values(value)[0]}</div>
                                                )
                                            })
                                        }
                                    </Col>
                                ...
                                <Row>
                                    <Col md={12} style={{textAlign: 'center'}}><b>Senarai Resit
                                        Bayaran</b></Col>
                                    <Col>
