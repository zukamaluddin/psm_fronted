...
export default class WizardStep4 extends React.Component {
    ...
    async onLoad() {
        ...
        if (data) {
            ...
            this.setState({statusDesc: 'Pendaftaran Pemilik telah disimpan.'});
        } else {
            this.setState({blocking: false});
            this.setState({statusAPI: true});
            this.setState({statusDesc: 'Pendaftaran Pemilik tidak berjaya.'});
        }
    }
    ...
    onSubmit = async () => {
        ...
        return new Promise((resolve, reject) => {
            ...
            fetch(`${global.ipServer}owner/create/${global.global_id}?token=${global.token}`, {
                method: 'POST',
                body: formData,
                headers: {
                        ...
                        <div className="no-results">
                            ...
                            {(!this.state.statusAPI) ?
                                <div className="text-center">
                                    <Button style={{width: '140px'}} onClick={this.handleSubmit} color="success"
                                            size="lg"
